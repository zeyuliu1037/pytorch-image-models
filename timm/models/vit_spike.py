...
from .layers import Mlp, DropPath, to_2tuple, trunc_normal_, HoyerBiAct
...
class LocallyGroupedAttn(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, attn_drop=0., proj_drop=0., ws=1):
        ...
        self.normq = nn.BatchNorm2d(self.num_heads)
        self.actq = HoyerBiAct(num_features=self.num_heads, spike_type='cw')
    ...
    def forward(self, x, size: Size_):
        ...
        q, k, v = q.reshape(B*_h*_w, self.num_heads, self.ws*self.ws, C//self.num_heads), \
        ...
        x = x.permute(2, 1).reshape(B, self.num_heads, C // self.num_heads, N)
...
@register_model
def twins_pcpvt_small_spike(pretrained=False, **kwargs):
    model_kwargs = dict(
