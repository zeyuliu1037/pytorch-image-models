...
class LocallyGroupedAttn(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, attn_drop=0., proj_drop=0., ws=1):
        ...
        self.norm_q = nn.BatchNorm2d(self.num_heads)
        self.act_q = HoyerBiAct(num_features=self.num_heads, spike_type='cw')
        self.norm_k = nn.BatchNorm2d(self.num_heads)
        self.act_k = HoyerBiAct(num_features=self.num_heads, spike_type='cw')

        self.norm_qk = nn.BatchNorm2d(self.num_heads)
        self.act_qk = HoyerBiAct(num_features=self.num_heads, spike_type='cw')
    ...
    def forward(self, x, size: Size_):
        ...
        q, k = q.reshape(B*_h*_w, self.num_heads, self.ws*self.ws, C//self.num_heads), \
                k.reshape(B*_h*_w, self.num_heads, self.ws*self.ws, C//self.num_heads)
        q, k = self.act_q(self.norm_q(q)), self.act_k(self.norm_k(k))
        q, k = q.reshape(B, _h*_w, self.num_heads, self.ws*self.ws, C//self.num_heads), \
                k.reshape(B, _h*_w, self.num_heads, self.ws*self.ws, C//self.num_heads)

        attn = (q @ k.transpose(-2, -1))
        attn = attn.reshape(B*_h*_w, self.num_heads, self.ws*self.ws, self.ws*self.ws)
        attn = self.act_qk(self.borm_qk(attn))
        attn = attn.reshape(B, _h*_w, self.num_heads, self.ws*self.ws, self.ws*self.ws)
...
class GlobalSubSampleAttn(nn.Module):
    ...
    def forward(self, x, size: Size_):
        ...
        attn = self.act_qk(self.norm_qk(attn))
        attn = self.attn_drop(attn)
