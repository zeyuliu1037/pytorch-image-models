...
from .layers import PatchEmbed, Mlp_spike, DropPath, trunc_normal_, lecun_normal_, HoyerBiAct
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.act = HoyerBiAct(num_features=dim, spike_type='cw')

        self.conv_q = nn.Conv2d(dim, dim, kernel_size=1, stride=1,bias=False)
        self.conv_k = nn.Conv2d(dim, dim, kernel_size=1, stride=1,bias=False)
        self.norm_k = nn.BatchNorm2d(dim)
        self.act_k = HoyerBiAct(num_features=dim, spike_type='cw')
        self.conv_v = nn.Conv2d(dim, dim, kernel_size=1, stride=1,bias=False)
        self.norm_v = nn.BatchNorm2d(dim)
        self.act_v = HoyerBiAct(num_features=dim, spike_type='cw')
        
        self.norm_qkv = nn.BatchNorm2d(dim)
        self.act_qkv = HoyerBiAct(num_features=dim, spike_type='cw')
    ...
    def forward(self, x):
        ...
        x = self.act(self.norm(x)) # B, num_heads, N, C // num_heads
        q = self.conv_q(x)
        k, v = self.act_k(self.norm_k(self.conv_k(x))), self.act_v(self.norm_v(self.conv_v(x)))
        q, k, v = q.reshape(B, C, H*W).permute(0, 2, 1).reshape(B, H*W, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous(), \
                k.reshape(B, C, H*W).permute(0, 2, 1).reshape(B, H*W, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous(), \
                v.reshape(B, C, H*W).permute(0, 2, 1).reshape(B, H*W, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, H*W, C).permute(0, 2, 1).reshape(B, C, H, W)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = Mlp_spike(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, drop=drop)
    ...
    def __init__(
        ...
        for _ in range(num_parallel):
            ...
            self.ffns.append(nn.Sequential(OrderedDict([
                ('norm', norm_layer(dim)),
                ('mlp', Mlp_spike(dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, drop=drop)),
                ('ls', LayerScale(dim, init_values=init_values) if init_values else nn.Identity()),
                ('drop_path', DropPath(drop_path) if drop_path > 0. else nn.Identity())
            ])))
...
class VisionTransformer_Spike(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        self.patch_embed = embed_layer(
            ...
            flatten=False, # output x: B, C, H, W
            norm_layer=nn.BatchNorm2d, # add bn
        )
    ...
    def _pos_embed(self, x):
        ...
        if self.no_embed_class:
            ...
            x = x + self.pos_embed
            if self.cls_token is not None:
                print(('cls_token1: {}, x: {}'.format((self.cls_token.expand(x.shape[0], -1, -1)).shape, x.shape)))
                exit()
            ...
            if self.cls_token is not None:
                print(('cls_token2: {}, x: {}'.format((self.cls_token.expand(x.shape[0], -1, -1)).shape, x.shape)))
                exit()
                x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)
            x = x + self.pos_embed
...
@torch.no_grad()
def _load_weights(model: VisionTransformer_Spike, checkpoint_path: str, prefix: str = ''):
...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(
        VisionTransformer_Spike, variant, pretrained,
...
@register_model
def vit_small_patch16_224_sipke_q(pretrained=False, **kwargs):
    """ ViT-Small (ViT-S/16)
    NOTE I've replaced my previous 'small' model definition and weights with the small variant from the DeiT paper
    """
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, **kwargs)
    model = _create_vision_transformer('vit_small_patch16_224_sipke_q', pretrained=pretrained, **model_kwargs)
    return model
